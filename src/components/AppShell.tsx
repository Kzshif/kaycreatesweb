...
export default function AppShell({
  ...
  return (
    <div className="flex min-h-screen">
      {/* Sidebar */}
      <aside className="sticky top-0 hidden h-screen w-64 shrink-0 flex-col bg-ink p-4 text-paper md:flex">
        <Link href="/app" className="flex items-center gap-2.5 px-2 py-3">
          <span className="grid h-9 w-9 place-items-center rounded-xl bg-primary font-display text-lg font-bold">
            K
          </span>
        ...
        <div className="mt-2 rounded-xl bg-white/5 px-3.5 py-3">
          ...
          <p className="mt-0.5 text-xs text-paper/50">{planBadge}</p>
        </div>
        ...
        <div className="border-t border-white/10 pt-3">
          ...
          <p className="truncate px-2 text-xs text-paper/45">{user.email}</p>
          <button
            onClick={logout}
            className="mt-2 w-full rounded-xl px-2 py-2 text-left text-sm text-paper/60 transition hover:bg-white/10 hover:text-paper"
          >
            ← Sign out
          </button>
        </div>
